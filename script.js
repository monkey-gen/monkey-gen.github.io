...
// this section will add "copy" buttons next to each element that has a "copy" class. It is backwards compatible to most browsers. It will check if the element has an ID already, otherwise it will assign one. 
function copyToClipboard(fieldindex,copybutton,target) {
//fieldindex is unused now 	
//&#128203;clipboard  	&#128461;empty pages	&#128464;pages &#10003; checkmark
var docelement = document.getElementById(target);  
var htmltag = docelement.tagName.toLowerCase(); 
//alert(htmltag);
if( htmltag == 'code' || htmltag == 'textarea'  || htmltag == 'pre' || htmltag == 'a' || htmltag == 'span' || htmltag == 'div'  ) {var text = docelement.innerHTML ; }
if( htmltag == 'input') {var text = docelement.value ; }
if(text  == '' || text  == '-') { 
text = '-'; 
// alert('Field is empty! Nothing to copy!'); return; 
copybutton.innerHTML  = 'X';
copybutton.style.color = "#ec001c";

} else {
copybutton.innerHTML  = '&#10003;';
copybutton.style.color = "#9aff5d";
...
var i; for (i = 0; i < copyelements.length; i++) {

if(copyelements[i].id) {
	var target = copyelements[i].id;
} 
else {
	var target = "copy" + i ;
	copyelements[i].id = target; //works only with unreferenced html tags
}

document.getElementById(target).insertAdjacentHTML('afterend', '<span class="copybutton" id="copybutton' + i + '" onclick="copyToClipboard(\'' + i + '\',this,\'' + target + '\')" title="copy to clipboard">&nbsp;</span>');
}
}
...
function validateAddress(address) {
    let re, isABananoAddress;
    return new RegExp("^ban_[013-9a-km-uw-z]{60}$").test(address);
}

const monKeyImageEndpoint = "https://monkey.banano.cc/api/v1/monkey/";

function AsynchronouslyDownloadAMonkeyImage(address, img) {
    img.src = "loading.gif";
...
function generateMonKey() {
    let bananoAddress = fieldaddress.value;
    if (validateAddress(bananoAddress)) {
        let img;
        AsynchronouslyDownloadAMonkeyImage(bananoAddress, document.getElementById("generated-monKey"));
    };
}
...
window.onload = function () { loadCopyHelper(); }
